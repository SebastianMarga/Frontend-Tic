...
export const stockService = {
  async getMovements(filters = {}) {
    if (USE_MOCK) {
      ...
      if (filters.type && filters.type !== "Todos los tipos" && filters.type !== "Todos") {
        data = data.filter((m) => m.type.toLowerCase() === filters.type.toLowerCase());
      }
  ...
  async registerStockIn({ productId, quantity, batchNumber, expDate, note = "Entrada manual de stock" }) {
    if (USE_MOCK) {
      ...
      if (productIndex !== -1) {
        ...
        if (!products[productIndex].batches) products[productIndex].batches = [];
        
        const newBatch = {
          id: `LT-${Date.now().toString().slice(-4)}`,
          batchNumber: batchNumber || `L-${new Date().getFullYear()}-${Math.floor(100 + Math.random() * 900)}`,
          quantity: qtyNum,
        ...
        // Update status
        if (products[productIndex].currentStock > products[productIndex].dynamicThreshold) {
      ...
      const newMovement = {
        id: `MOV-${Date.now()}`,
        dateTime: new Date().toLocaleDateString("es-ES", { day: "2-digit", month: "2-digit" }) + " " + new Date().toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" }),
        type: "Entrada",
  ...
  async registerStockOut({ productId, quantity, reason = "Despacho manual" }) {
    if (USE_MOCK) {
      ...
      let productObj = null;
      if (productIndex !== -1) {
        products[productIndex].currentStock = Math.max(0, products[productIndex].currentStock - qtyNum);
        ...
        } else if (products[productIndex].currentStock <= products[productIndex].dynamicThreshold) {
